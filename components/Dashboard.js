        ...
        <div className="left flex flex-col justify-between text-white bg-[#18191c] w-1/4 float-left py-5">
            <div className="dashboard py-5 mt-20 w-full">
                <h2 className="py-1.5 px-5 text-gray-400">Saved Files</h2>
                    {isLoading ? <Loader/> : 
                    <div>
                    {files.length === 0 ? (<p className="text-xs mt-10 text-center text-gray-400">Files are empty</p>) : (
                        ...
                        {files.map((file) => (
                            ...
                            <DeleteButton setIsLoading={setIsLoading} refreshFiles={refreshFiles} fileId={file._id} fileName={file.fileName} />
                        </span>
                    ))}
                    </div>
                    )}
